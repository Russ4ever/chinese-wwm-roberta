...
shard_tag = sys.argv[1]; tasks_json = sys.argv[2]
task_ids = json.loads(Path(tasks_json).read_text())
config = load_layer_probe_config(ROOT/"configs"/"layer_probe_walk_forward.yaml")
t0 = time.time()
print(f"[shard {shard_tag}] start tasks={len(task_ids)} ids={task_ids}", flush=True)
d = run_walk_forward_probe_stage(config, task_ids=task_ids, shard_tag=shard_tag)
print(f"[shard {shard_tag}] DONE in {(time.time()-t0)/60:.1f} min -> {d}", flush=True)
